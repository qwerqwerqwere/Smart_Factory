...
' SSL 인증서 검사를 패스하도록 설정
http.setOption 2, 13056 ' Ignore SSL certificate errors
...
For i = 0 To UBound(devices, 1)
    ...
    lineProtocol = measurement & "," & tags & " " & fields & vbCrLf

        http.setRequestHeader "Authorization", "Token " & token
        http.setRequestHeader "Content-Type", "text/plain; charset=utf-8"

        On Error Resume Next
        http.Send currentBatch
        If Err.Number <> 0 Then
        ...
        If http.Status <> 204 Then
            MsgBox "Batch 전송 실패. HTTP Status: " & http.Status & " - " & http.responseText
        Else
            'MsgBox "Batch 전송 성공"
        End If
